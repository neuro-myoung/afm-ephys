...
p1 <- ggplot(dat_triage, aes(x=construct, y=thresh, fill=construct, colour=construct)) +
  ...
  geom_errorbar(data = aggthresh_dat, aes(ymin = mean-sem, ymax = mean+sem, y = mean), 
                size=0.4, width=0,alpha=0.8, colour='black')+
  geom_point(data=aggthresh_dat, aes(construct, mean+sem),shape=16,size=0.05, alpha=0.8,colour='black')+
  geom_point(data=aggthresh_dat, aes(construct, mean-sem),shape=16,size=0.05, alpha=0.8,colour='black')+
  geom_point(data=aggthresh_dat, aes(construct, mean), fill='white',shape=21,size=1.5, stroke=0.25, colour='black')+
  geom_beeswarm(shape = 21, size = 0.75, alpha = 0.8,cex=1, stroke=0.25, 
                   colour='black')+
...
p2 <- ggplot(dat_triage, aes(x=construct, y=Cm, fill=construct, colour=construct)) +
  geom_violin(alpha=0.6, colour='gray28', lwd=0.25, outlier.shape=NA,scale='width')+
  geom_errorbar(data = aggcap_dat, aes(ymin = mean-sem, ymax = mean+sem, y = mean), 
                size=0.4, width=0,alpha=0.8, colour='black')+
  geom_point(data=aggcap_dat, aes(construct, mean+sem),shape=16,size=0.05, alpha=0.8,colour='black')+
  geom_point(data=aggcap_dat, aes(construct, mean-sem),shape=16,size=0.05, alpha=0.8,colour='black')+
  geom_point(data=aggcap_dat, aes(construct, mean), fill='white',shape=21,size=1.5, stroke=0.25, colour='black')+
  geom_beeswarm(shape = 21, size = 0.75, alpha = 0.8,cex=1, stroke=0.25, 
                   colour='black')+
  scale_x_discrete(breaks=c('mp1','mp2','mtraak','mtrek1','mscl','osca12','tmem63a','tmem63b',
                            'trpa1','trpv4','pkd2l1','yfp'), labels = names)+
...
plot_grid(p1,NULL,p2, align = 'vh', rel_heights=c(1,-0.2,1),nrow=3)

ggsave('agg_capthresh.pdf', plot=last_plot(), dpi=300, width=5, height=6, units= "in", dev=cairo_pdf)
